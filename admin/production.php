...
<br>
<p>Добавьте новую статью в раздел "Продукция" или с помощью поиска по заголовку выведете существующие для удаления или редактирования.<br>
...
if(isset($_POST["search"])){
  ...
  $query = "SELECT * FROM production WHERE tema LIKE '%$search%' ORDER by id DESC";
...
if(isset($_POST['insert'])){
  $query="INSERT INTO production (tema,text) VALUES ('$_POST[tema]','$_POST[text]')";
  mysql_query($query);
}
//Корректировка
if(isset($_POST['edit'])){
  $query="UPDATE production SET tema='$_POST[tema]',text='$_POST[text]'  WHERE id='$_POST[id]'";
  $result = mysql_query($query);
  unset($row);  
}
//Удаление
if(isset($_POST['delete'])){
  $query="DELETE FROM production WHERE id='$_POST[id]'";
  mysql_query($query);
  unset($row);
}
...
<tr><td>Статья</td><td><textarea name="text" cols=160 rows=40>$row[text]</textarea>
...
<tr><td>Статья</td><td><textarea name="text" cols=180 rows=40></textarea>
<script type="text/javascript">
CKEDITOR.replace( 'text');
</script>
</td></tr>
